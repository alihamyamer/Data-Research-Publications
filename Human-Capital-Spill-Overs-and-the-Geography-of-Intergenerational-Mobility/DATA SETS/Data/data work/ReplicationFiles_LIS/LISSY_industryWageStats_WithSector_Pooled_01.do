...
local pVariables hid ppopwgt sex age emp educ gross1 weeks ind1_c hours sector1


**Select output variable
*local outputVar r_wsd
...
save $mydata\bhacki_industrystats, replace
...
foreach cc in `pools' {
	...
	*********************************Do pooled analysis here*********************************
	capture noisily {	
		...
		*Drop workers with missing wage or industry code
		drop if wage==0 | wage==. | ISIC==.

		* Keep workers of age 16-65
		keep if age>=16 & age<=65
		
		*Keep if working in the private sector
		keep if sector1 < 2000
		...
		*NOTE: Include self-employment hours?
		generate hourswgt = ppopwgt*hours
		*Define industry-specific final weights
		egen indwgt = sum(hourswgt), by(ISIClab)
		*Mean weighted wage in each industry
		gen wbarTemp = (wage*hourswgt)/indwgt
		egen wbar = sum(wbarTemp), by(ISIClab)

		***Generate industry-specific weighted variance and CV
		gen dev2 = (wage-wbar)^2*(hourswgt/indwgt)
		...
		***Generate industry labour shares***
		egen totalwgt = sum(hourswgt)
		gen indLabourShare = indwgt/totalwgt
		...
		*xi, noomit: reg wage hcap i.ISIClab i.state i.smsastat age age2 age3 i.sex i.veteran i.race i.ethnic i.prcitshp i.class94 [fw=weightr], r 
		gen age2 = age*age
		gen age3 = age2*age
		quietly xi, noomit: reg wage i.educ i.ISIClab age age2 age3 i.sex i.cname*i.region_c  i.year [pw=ppopwgt], r 
		predict r_wage, res

		***Repeat construction of dispersion measures from regression residual
		***Construct industry-specific weighted mean
		*NOTE: Include self-employment hours?
		generate r_hourswgt = ppopwgt*hours if r_wage !=.
		*Define industry-specific final weights
		egen r_indwgt = sum(r_hourswgt), by(ISIClab)
		*Mean weighted wage in each industry
		gen r_wbarTemp = (r_wage*r_hourswgt)/r_indwgt
		egen r_wbar = sum(r_wbarTemp), by(ISIClab)

		***Generate industry-specific weighted variance and CV
		gen r_dev2 = (r_wage-r_wbar)^2*(r_hourswgt/r_indwgt)
		egen r_varwage = sum(r_dev2), by(ISIClab)
		gen r_wsd = sqrt(r_varwage)
		gen r_cvwage = r_wsd/r_wbar

		***Generate industry specific 95th-5th percentiles
		egen r_p5 = pctile(r_wage), p(5) by(ISIClab)
		egen r_p95 = pctile(r_wage), p(95) by(ISIClab)
		gen r_wagepctile = r_p95-r_p5
		gen r_wagepctilen = r_wagepctile/r_wbar

		***Generate industry labour shares***
		egen r_totalwgt = sum(r_hourswgt)
		gen r_indLabourShare = r_indwgt/r_totalwgt
		
		
		***Save vars for merging***
		egen `cc'_wobs = count(wbar), by(ISIClab)
		gen `cc'_wm = wbar
		gen `cc'_wsd = sdwage
		egen `cc'_rwobs = count(r_wbar), by(ISIClab)
		gen `cc'_rwm = wbar
		gen `cc'_rwsd = sdwage
		
		collapse `cc'_wobs `cc'_wm `cc'_wsd `cc'_rwobs `cc'_rwm `cc'_rwsd, by(ISIClab) fast
		
		keep `cc'_wobs `cc'_wm `cc'_wsd `cc'_rwobs `cc'_rwm `cc'_rwsd ISIClab
		sort ISIClab

		merge 1:m ISIClab using $mydata\bhacki_industrystats
...
******* PRINT results *******
use $mydata\bhacki_industrystats, clear

tabstat *_wobs *_wm *_wsd *_rwobs *_rwm *_rwsd, by(ISIClab) nototal format(%9.3g)
